...
impl DateTime {
    ...
    pub fn format(&self, fmt: &str) -> String {
        fmt.replace("%D", &format!("{:02}/{:02}/{:02}", self.month(), self.day(), self.year() % 100))
            ...
            .replace("%A", &WEEKDAY_NAMES[if self.day_of_week() == 0 { 6 } else { self.day_of_week() as usize - 1 }])
            .replace("%a", &SHORT_WEEKDAY_NAMES[if self.day_of_week() == 0 { 6 } else { self.day_of_week() as usize - 1 }])
